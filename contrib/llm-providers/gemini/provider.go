...
func init() {
	// 自动注册 Gemini provider 到全局注册表
	registry.Register(constants.ProviderGemini, func(opts ...agentllm.ClientOption) (agentllm.Client, error) {
		return New(opts...)
	})
}

// Provider implements LLM interface for Google Gemini
...
// New creates a new Gemini provider using options pattern
...
func (p *Provider) convertToolsToFunctions(tools []interfaces.Tool) []*genai.FunctionDeclaration {
...
func (p *Provider) toolSchemaToGeminiSchema(schema interface{}) *genai.Schema {
...
func (p *StreamingProvider) StreamWithContext(ctx context.Context, prompt string) (<-chan StreamEvent, error) {
